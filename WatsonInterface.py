    ...
    def createCrimeListObjects(self):
            ...
            for query in results:
              for i in range(len(query["results"])):
                ...
                reportDate = datetime.datetime.strptime(dates[0], date_format_string)
                crimeStart =  datetime.datetime.strptime(dates[1], date_format_string)
                crimeEnd = None if len(dates) < 3 else datetime.datetime.strptime(dates[2], date_format_string)
                #Some crimes have multiple offenses which are usually separated by ';' character
                #This is not guaranteed as formatting is a bit different for some crime reports
                offenseList = (query["results"][i]["question"][0]).split(';')
                location = query["results"][i]["subtitle"][0]
                disposition = query["results"][i]["author"][0]
                crime = CrimeReport(crimeID, reportDate, crimeStart, crimeEnd, offenseList, location, disposition)
